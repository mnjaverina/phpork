<?php
...

	{
		...
		/*
		for the search by house function
		*/
		public function getPigsByHouse($house)
		{
				$link = $this->connect();
				$pquery = "SELECT p.pig_id,
							rt.label
							FROM pig p
							INNER JOIN rfid_tags rt ON rt.pig_id = p.pig_id
							INNER JOIN pen pen ON p.pen_id = pen.pen_id
							INNER JOIN house h ON h.house_id = pen.house_id
		...
		/*
			search pigs by location
		*/
		public function getPigsByLocation($loc)
		{
				$link = $this->connect();
				$pquery = "SELECT p.pig_id,
							rt.label
